...
typedef void (*algo_open)(void);
typedef void (*algo_close)(void);
...
EXTERNC bool smf_msg_service_open(void);

EXTERNC bool smf_msg_service_close(void);
...
EXTERNC void smf_local_player_config(void);
...
EXTERNC bool smf_local_player_stop(void);

EXTERNC bool smf_local_player_pause(void);

EXTERNC bool smf_local_player_getMediaFromPath(const char* path,smf_meta_info_t* pMeta, struct local_play_meida_info *pMedia);

EXTERNC bool smf_local_player_getMusicPosition(uint32_t* pos);

EXTERNC bool smf_local_player_next(void);

EXTERNC bool smf_local_player_previous(void);
...
EXTERNC void smf_tws_config(void);
/*-----------------LOCAL PLAY DEMO----------------*/
EXTERNC bool smf_local_player_start_test(void);
/*-----------------LOCA PLAY EXTRA API----------------*/
EXTERNC bool smf_local_player_start_with_param(void);
...
EXTERNC WEAK bool smf_local_player_unregister_notify(void);
#endif
