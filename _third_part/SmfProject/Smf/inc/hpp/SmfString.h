...
namespace smf {
	...
	constexpr inline uint32_t Swap24(const uint32_t v) {
	...
	constexpr inline uint32_t Read24(const void* ptr) {
		return ((uint32_t)((uint8_t*)ptr)[0] | ((uint32_t)((uint8_t*)ptr)[1] << 8) | ((uint32_t)((uint8_t*)ptr)[2] << 16));
	}
	...
	constexpr inline uint16_t Read16BE(const void* ptr) {
		return ((uint16_t)((uint8_t*)ptr)[1] | ((uint16_t)((uint8_t*)ptr)[0] << 8));
	}
	constexpr inline uint32_t Read24BE(const void* ptr) {
		return ((uint32_t)((uint8_t*)ptr)[2] | ((uint32_t)((uint8_t*)ptr)[1] << 8) | ((uint32_t)((uint8_t*)ptr)[0] << 16));
	}
	constexpr inline uint32_t Read32BE(const void* ptr) {
		return ((uint32_t)((uint8_t*)ptr)[3] | ((uint32_t)((uint8_t*)ptr)[2] << 8) | ((uint32_t)((uint8_t*)ptr)[1] << 16) | ((uint32_t)((uint8_t*)ptr)[0] << 24));
	}
	constexpr inline uint64_t Read64BE(const void* ptr) {
		return ((uint64_t)Read32BE(ptr) << 32) | Read32BE((uint32_t*)ptr + 1);
	}
	///
	...
	constexpr inline void Write24(void* ptr, uint32_t val) {
		((uint8_t*)ptr)[0] = val & 0xff;
	...
	constexpr inline void Write16BE(void* ptr, uint16_t val) {
		((uint8_t*)ptr)[0] = val >> 8;
		((uint8_t*)ptr)[1] = val & 0xff;
	}
	constexpr inline void Write24BE(void* ptr, uint32_t val) {
		((uint8_t*)ptr)[0] = (val >> 16) & 0xff;
		((uint8_t*)ptr)[1] = (val >> 8) & 0xff;
		((uint8_t*)ptr)[2] = val & 0xff;
	}
	constexpr inline void Write32BE(void* ptr, uint32_t val) {
		Write16BE((uint16_t*)ptr, val >> 16);
		Write16BE((uint16_t*)ptr + 1, val & 0xffff);
	}
	constexpr inline void Write64BE(void* ptr, uint64_t val) {
		Write32BE((uint32_t*)ptr, val >> 32);
		Write32BE((uint32_t*)ptr + 1, val & 0xffffffff);
	}
	...
	static inline const char* splite(char*& ptr, char ch) {
		auto ptr0 = ptr;
		while (*ptr && *ptr != ch)ptr++;
		return ptr0;
	}
	static inline const char* splite(char*& ptr, char ch, char* end) {
		auto ptr0 = ptr;
		if (end)
			while (*ptr && *ptr != ch && ptr < end)ptr++;
		else 
			while (*ptr && *ptr != ch)ptr++;
		return ptr0;
	}
	static inline uint32_t splite(char*& ptr, char ch, const char** outputs, int max, char* end = 0) {
		for (int i = 0; i < max; i++) {
			auto key = outputs[i] = splite(ptr, ch, end);
			if (!*ptr || *ptr != ch || key == ptr)return i + 1;
			ptr++;
		}
		return max;
	}
