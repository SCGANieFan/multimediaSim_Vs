...
namespace smf {
	...
	struct TFcc {
		...
		static constexpr inline T fcc(const char* s, uint8_t c = sizeof(T), T v = 0, uint8_t i = 0) {
			return (s && (i < c) && IsChar(*s)) ? fcc(fcc(s + 1, c, v, i + 1), *s) : 0;
		}
		static constexpr inline T fccall(const char* s, uint8_t c = sizeof(T), T v = 0, uint32_t i = 0) {
			return (s && (i < c)) ? fcc(fccall(s + 1, c, v, i + 1), *s) : 0;
		}
	...
	//
	constexpr uint32_t fcc32(const char*s, uint8_t c = 4) {
		return TFcc<uint32_t, XOther>::fcc(s, c);
	}
	constexpr uint32_t fcc32up(const char*s, uint8_t c = 4) {
		return TFcc<uint32_t, XUpper>::fcc(s, c);
	}
	constexpr uint32_t fcc32low(const char*s, uint8_t c = 4) {
		return TFcc<uint32_t, XLower>::fcc(s, c);
	}
	constexpr uint64_t fcc64(const char* s, uint8_t c = 8) {
		return TFcc<uint64_t, XOther>::fcc(s, c);
	}
	constexpr uint64_t fcc64up(const char*s, uint8_t c = 8) {
		return TFcc<uint64_t, XUpper>::fcc(s, c);
	}
	constexpr uint64_t fcc64low(const char*s, uint8_t c = 8) {
		return TFcc<uint64_t, XLower>::fcc(s, c);
	}
	...
	constexpr uint32_t fccall32(const char* s, uint8_t c = 4) {
		return TFcc<uint32_t, XOther>::fccall(s, c);
	}
	constexpr uint32_t fccall32up(const char* s, uint8_t c = 4) {
		return TFcc<uint32_t, XUpper>::fccall(s, c);
	}
	constexpr uint32_t fccall32low(const char* s, uint8_t c = 4) {
		return TFcc<uint32_t, XLower>::fccall(s, c);
	}
	constexpr uint64_t fccall64(const char* s, uint8_t c = 8) {
		return TFcc<uint64_t, XOther>::fccall(s, c);
	}
	constexpr uint64_t fccall64up(const char* s, uint8_t c = 8) {
		return TFcc<uint64_t, XUpper>::fccall(s, c);
	}
	constexpr uint64_t fccall64low(const char* s, uint8_t c = 8) {
		return TFcc<uint64_t, XLower>::fccall(s, c);
	}
#if 0
