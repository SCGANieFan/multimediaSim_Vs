    ...
    int (*write_callback)(int64_t offset, const void* buffer, size_t size, void* token))
{
    ...
    minimp4_vector_init(&mux->tracks, 4 * sizeof(track_t));
    return mux;
}
...
int MP4E_add_track(MP4E_mux_t* mux, const MP4E_track_t* track_data)
{
    ...
    tr->trunk_id = ntr;
    minimp4_vector_t* smpl = mux->enable_fragmentation ? &tr->fragments : &tr->smpl;
    if (!minimp4_vector_init(smpl, 256)) {
        return MP4E_STATUS_NO_MEMORY;
    }
...
int MP4E_set_extra_data(MP4E_mux_t* mux, int track_id, const void* data, int bytes)
{
    track_t* tr = ((track_t*)mux->tracks.data) + track_id;
    return append_mem(&tr->extra, data, bytes) ? MP4E_STATUS_OK : MP4E_STATUS_NO_MEMORY;
}

static unsigned get_duration(const track_t* tr)
{
    return tr->duration;
    /*if (tr->info.fixed_duration) {
        return tr->info.default_duration * tr->frame_count;
    }
...
static int add_sample_descriptor(MP4E_mux_t* mux, track_t* tr, int data_bytes, int duration, int kind)
{
    ...
    return minimp4_vector_put(&tr->smpl, &smp, sizeof(sample_t)) ? MP4E_STATUS_OK : MP4E_STATUS_NO_MEMORY;
}

static int add_fragments_descriptor(MP4E_mux_t* mux, track_t* tr, int data_bytes, uint32_t duration, int kind)
{
    int err = MP4E_STATUS_OK;
    if (kind == MP4E_SAMPLE_RANDOM_ACCESS) {
        frag_t frag;
        frag.timestamp = tr->duration;//(duration ? duration : tr->info.default_duration);
        frag.offset = mux->write_pos;
        //tr->position = mux->write_pos;
        dbgTestPXL("%u,%u,%u", tr->duration, (uint32_t)mux->write_pos, tr->fragments_count);
        err = minimp4_vector_put(&tr->fragments, &frag, sizeof(frag_t)) ? MP4E_STATUS_OK : MP4E_STATUS_NO_MEMORY;
    }
    tr->fragments_count++;
    tr->duration += duration;
    return err;
...
static int mp4e_write_mfra_box(MP4E_mux_t* mux) {
    unsigned int ntr, /*index_bytes,*/ ntracks = mux->tracks.bytes / sizeof(track_t);
    uint32_t mfra_max = 8 + 12;
    for (ntr = 0; ntr < ntracks; ntr++)
    {
        track_t* tr = ((track_t*)mux->tracks.data) + ntr;
        mfra_max += 12 + 12 + tr->fragments.bytes * 2;
    }

    unsigned char* base = (unsigned char*)malloc(mfra_max);
    ...
    {
        track_t* tr = ((track_t*)mux->tracks.data) + ntr;
        minimp4_vector_t* vect = &tr->fragments;
        frag_t* frags = (frag_t*)vect->data;
        frag_t* frags_end = (frag_t*)(vect->data + vect->bytes);
        uint32_t number_entry = vect->bytes / sizeof(frag_t);
        ATOM_FULL(BOX_tfra, 0)
        WRITE_4(ntr + 1);  // track id
        WRITE_4(0);  // _reserved
        WRITE_4(number_entry);  // _number_entry
        while (frags < frags_end) {
            WRITE_4(frags->timestamp);//time
            WRITE_4(frags->offset);//moof_offset
            WRITE_3(0x010101);
            frags++;
        }
        END_ATOM
        //dbgTestPXL("%u,%u,%u", number_entry, frags_end - frags, tr->fragments_count);
    }
    ATOM_FULL(BOX_mfro, 0)
   // dbgTestPXL("%u/%u",p - base + 4, mfra_max);
    WRITE_4(p - base + 4);//mux->size
    END_ATOM
    END_ATOM

    uint32_t mfra_size = p - base;
    ERR(mux->write_callback(mux->write_pos, base, mfra_size, mux->token));
    mux->write_pos += mfra_size;
    free(base);
    return MP4E_STATUS_OK;
}

static uint32_t mp4e_get_duration(MP4E_mux_t* mux) {
    unsigned movie_duration = 0;
    uint32_t ntracks = mux->tracks.bytes / sizeof(track_t);
    for (int i = 0; i < ntracks; i++) {
        track_t* tr = ((track_t*)mux->tracks.data) + i;    // take 1st track
        unsigned duration = get_duration(tr);
        duration = (unsigned)(duration * 1LL * MOOV_TIMESCALE / tr->info.time_scale);
        if (movie_duration < duration)movie_duration = duration;
        //dbgTestPXL("[%u]%u=%u*%u", i, movie_duration, tr->info.default_duration, tr->frame_count);
    }
    return movie_duration;
}

static int mp4e_update_duration(MP4E_mux_t* mux) {
    int err = MP4E_STATUS_OK;
    if (mux->duration_pos) {
        uint32_t dura = mp4e_get_duration(mux);
        uint32_t duration = 0;
        WR4((char*)&duration, dura);
        err = mux->write_callback(mux->duration_pos, &duration, 4, mux->token);
        //dbgTestPXL("%u/%u", mux->duration_pos, duration);
    }
    return err;
}

static int mp4e_update_timestamp(MP4E_mux_t* mux) {
    int err = MP4E_STATUS_OK;
    track_t* tr = (track_t*)mux->tracks.data;
    track_t* tr_end = (track_t*)(mux->tracks.data + mux->tracks.bytes);
    while (tr < tr_end) {
        if (tr->first_timestamp_pos) {
            uint32_t timestamp = 0;
            WR4((char*)&timestamp, tr->first_timestamp);
            err = mux->write_callback(tr->first_timestamp_pos, &timestamp, 4, mux->token);
        }
        //dbgTestPXL("[%u]%u/%u", tr->trunk_id, tr->first_timestamp_pos, tr->first_timestamp);
        tr++;
    }
    return err;
}
int mp4e_put_sample_fragments(MP4E_mux_t* mux, int track_num, const void* data, int data_bytes, int duration, int kind) {
     track_t * tr = ((track_t*)mux->tracks.data) + track_num;
    if (kind) {
        ERR(add_fragments_descriptor(mux, tr, data_bytes, duration, kind));
...
*/
int MP4E_put_sample(MP4E_mux_t* mux, int track_num, const void* data, int data_bytes, int duration, int kind, uint32_t timestamp)
{
    track_t* tr;
    if (!mux || !data)
        return MP4E_STATUS_BAD_ARGUMENTS;
    ...
    if (!tr->frame_count++)
        tr->first_timestamp = timestamp;
    if (!duration || tr->info.fixed_duration)
        duration = tr->info.default_duration;
    ...
    if (kind != MP4E_SAMPLE_CONTINUATION)
    {
        ...
        ERR(add_sample_descriptor(mux, tr, data_bytes, duration, kind));
    }
    else
    {
        if (!mux->sequential_mode_flag)
...
int MP4E_set_text_comment(MP4E_mux_t* mux, const char* comment)
{
    ...
    mux->text_comment_len = strlen(comment) + 1;
    mux->text_comment = (char*)malloc(mux->text_comment_len);
    if (!mux->text_comment)
        return MP4E_STATUS_NO_MEMORY;
    memcpy(mux->text_comment, comment, mux->text_comment_len);
    return MP4E_STATUS_OK;
}

int MP4E_set_text_comment_ext(MP4E_mux_t* mux, const void* comment, int size)
{
    if (!mux || !comment)
        return MP4E_STATUS_BAD_ARGUMENTS;
    if (mux->text_comment)
        free(mux->text_comment);
    //mux->text_comment = strdup(comment);
    mux->text_comment_len = size;
    mux->text_comment = (char*)malloc(size);
    if (!mux->text_comment)
        return MP4E_STATUS_NO_MEMORY;
    memcpy(mux->text_comment, comment, size);
    return MP4E_STATUS_OK;
}
...
#define FILE_HEADER_BYTES 256 + 15360 * 2
#define TRACK_HEADER_BYTES 512
    index_bytes = FILE_HEADER_BYTES;
    if (mux->text_comment)
        index_bytes += 32 + mux->text_comment_len;
    for (ntr = 0; ntr < ntracks; ntr++)
    {
        track_t* tr = ((track_t*)mux->tracks.data) + ntr;
        ...
        index_bytes += tr->extra.bytes;

        ERR(write_pending_data(mux, tr));
    }
    //dbgTestPDL(index_bytes);
    base = (unsigned char*)malloc(index_bytes);
    if (!base)
        return MP4E_STATUS_NO_MEMORY;
    p = base;

    if (!mux->sequential_mode_flag && !mux->enable_fragmentation)
    {
...
//#define MOOV_TIMESCALE 1000
    ...
    unsigned movie_duration = mp4e_get_duration(mux);
    if (ntracks)
    {        
        WRITE_4(MOOV_TIMESCALE); // duration
        WRITE_4(movie_duration); // duration
        //dbgTestPXL("%u,%u", movie_duration, MOOV_TIMESCALE);
    }
    ...
    for (ntr = 0; ntr < ntracks; ntr++)
    {
        ...
        //dbgTestPXL("%u,%u", samples_count, tr->smpl.bytes);
        if (mux->enable_fragmentation)
            samples_count = 0;
        ...
        tr->first_timestamp_pos = mux->write_pos + (p - base);
        WRITE_4(tr->first_timestamp);
        //WRITE_4(0);             // creation_time
        WRITE_4(0);             // modification_time
        ...
        if (tr->info.track_media_kind == e_private)
        {
            // mandatory private Media Header Box
            ATOM_FULL(BOX_nmhd, 0);
            END_ATOM;
        }
...
#define WRITE_OD_LEN(size) if (size > 0x7F) do { size -= 0x7F; WRITE_1(0x00ff); } while (size > 0x7F); WRITE_1(size)
                ...
                else
                {
                    ...
                    WRITE_3(256); // bufferSizeDB in bytes, constant as in reference decoder
                }
                //WRITE_3(tr->info.u.a.channelcount * 6144 / 8); // bufferSizeDB in bytes, constant as in reference decoder
        ...
        if (tr->info.track_media_kind == e_video && (MP4_OBJECT_TYPE_AVC == tr->info.object_type_indication || MP4_OBJECT_TYPE_HEVC == tr->info.object_type_indication))
        {
            ...
            else
            {
                ...
                WRITE_1((0 << 7) | (HEVC_NAL_VPS & 0x3f)); // Array Completeness + NAL Unit Type
                WRITE_2(numOfVPS);
                for (i = 0; i < tr->vvps.bytes; i++)
                {
                    WRITE_1(tr->vvps.data[i]);
                }
                WRITE_1((0 << 7) | (HEVC_NAL_SPS & 0x3f));
                WRITE_2(numOfSequenceParameterSets);
                for (i = 0; i < tr->vsps.bytes; i++)
                {
                    WRITE_1(tr->vsps.data[i]);
                }
                WRITE_1((0 << 7) | (HEVC_NAL_PPS & 0x3f));
        ...
        if (tr->extra.bytes >= 2) {
            ATOM(BOX_extr);
            for (i = 0; i < tr->extra.bytes-2; i++)
            {
                WRITE_1(tr->extra.data[i+2]);
            }
            END_ATOM;
        }
        END_ATOM;
...
#define MP4E_HANDLER_TYPE_MDIR  0x6d646972
        ...
        WRITE_4(1); // type
        WRITE_4(0); // lang
        for (i = 0; i < mux->text_comment_len; i++)
        {
    ...
    if (mux->enable_fragmentation)
    {
        ...
        //movie_duration = (unsigned)(movie_duration * 1LL * MOOV_TIMESCALE / tr->info.time_scale);
        ATOM(BOX_mvex);
        ATOM_FULL(BOX_mehd, 0);
        mux->duration_pos = mux->write_pos + (p - base);
        WRITE_4(movie_duration); // duration
        END_ATOM;
        for (ntr = 0; ntr < ntracks; ntr++)
        {
            ...
            WRITE_4(tr->info.default_duration);              // default_sample_duration
    ...
    uint32_t index_size = p - base;
    dbgTestPXL("%u,%u/%u",(uint32_t)mux->write_pos, index_size, index_bytes);
    assert((unsigned)(p - base) <= index_bytes);
    // dbgTestPXL("%u,%u/%u",(uint32_t)mux->write_pos, index_size, index_bytes);
    err = mux->write_callback(mux->write_pos, base, index_size, mux->token);
    free(base);
    mux->write_pos += index_size;
    return err;
}
...
int MP4E_close(MP4E_mux_t* mux)
{
    ...
    if (!mux->enable_fragmentation) {
        ...
        err = mp4e_write_mfra_box(mux);
        err = mp4e_update_timestamp(mux);
        err = mp4e_update_duration(mux);
    }
...
static int mp4_h265_write_nal(mp4_h26x_writer_t* h, const unsigned char* nal, int sizeof_nal, unsigned timeStamp90kHz_next)
{
    ...
    int err = MP4E_STATUS_OK;
    //dbgPrintf("payload_type=%d, intra=%d\n", payload_type, is_intra);
    ...
    default:
            ...
            return MP4E_STATUS_BAD_ARGUMENTS;
        {
            ...
            err = MP4E_put_sample(h->mux, h->mux_track_id, tmp, 4 + sizeof_nal, timeStamp90kHz_next, sample_kind,0);
            free(tmp);
        }
        break;
    }
        ...
        default:
            ...
            if (!h->need_pps && !h->need_idr)
            {
                ...
                if (first_mb_in_slice)
                    ...
                    sample_kind = MP4E_SAMPLE_RANDOM_ACCESS;
                err = MP4E_put_sample(h->mux, h->mux_track_id, nal2, sizeof_nal, timeStamp90kHz_next, sample_kind, 0);
            }
            break;
        }
        free(nal1);
