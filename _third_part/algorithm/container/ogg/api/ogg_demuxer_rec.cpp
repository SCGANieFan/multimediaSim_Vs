...
OggRet_t OggDeMuxer_c::Receive(GaapiData_c& iData){
	...
	uint8_t* iBuf = (uint8_t*)ogg_sync_buffer(&_oggSyncS, len);
	if (!iBuf) return OGG_API_RET_FAIL;
	gaapi_memcpy(iBuf, iData.Data(), len);
	ogg_sync_wrote(&_oggSyncS, len);
	...
	if (_stage == Stage_e::STAGE_AUDIO_DATA) {
		...
		if (sizeof(_audioDataHeadCache) < _oggPage.header_len)
			return OGG_API_RET_INNER_ERROR;
		//_pack10MsStep = _oggStreamS.granule_vals[_oggStreamS.lacing_fill - 1] / (441 * _oggStreamS.lacing_fill);
		_t10Ms = _oggStreamS.granule_vals[_oggStreamS.lacing_fill - 1] / 441;
...
OggRet_t OggDeMuxer_c::Generate(GaapiData_c& oData){
	...
	_granulepos = oggPacket.granulepos;
	_packetno = oggPacket.packetno;
	oData.Append(oggPacket.packet, oggPacket.bytes);
	if (_granulepos > 0) {
		//LOG_OGG("[%lld] %d,%lld", _packetno, oggPacket.bytes, _granulepos);
		//LOG_OGG("%lld m,%lld s,%lld ms", _t10Ms / 6000, _t10Ms / 100, _t10Ms *10);
	}
	return OGG_API_RET_SUCCESS;
}
