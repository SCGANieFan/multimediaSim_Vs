...
OpusRet_t OpusMSEnc_c::RunNoHead(OpusData_c& iData, OpusData_c& oData) {
    if (iData.Size() < _pcmFrameByte) {
        LOG_OPUS("%d < %d", iData.Size(), _pcmFrameByte);
        return OPUS_RET_FAIL;
    }
    int ret = opus_multistream_encode(_hd, (short*)iData.Data(), _pcmFrameSample, (unsigned char*)oData.LeftData(), oData.LeftSize());
    if (ret < 0) {
...
OpusRet_t OpusMSEnc_c::RrunWithHead(OpusData_c& iData, OpusData_c& oData) {
    ...
    int ret = opus_multistream_encode(_hd, (short*)iData.Data(), _pcmFrameSample, (uint8_t*)oData.LeftData() + headByte, oData.LeftSize() - headByte);
    if (ret < 0) {
        LOG_OPUS("enc fail, (%d,%s) (%p,%p,%d,%p,%d)",
            ret, ret2str[-ret], _hd, iData.Data(), iData.Size(), oData.LeftData(), oData.LeftSize());
        return OPUS_RET_FAIL;
    ...
    opus_multistream_encoder_ctl(_hd, OPUS_GET_FINAL_RANGE(&enc_final_range));
...
OpusRet_t OpusMSEnc_c::Open() noexcept {
    ...
    _hd = opus_multistream_encoder_create(
    ...
    opus_multistream_encoder_ctl(_hd, OPUS_SET_EXPERT_FRAME_DURATION(fd)); //OPUS_FRAMESIZE_ARG
    opus_multistream_encoder_ctl(_hd, OPUS_SET_FORCE_MODE(_encMode)); //OPUS_AUTO
    opus_multistream_encoder_ctl(_hd, OPUS_SET_BANDWIDTH(OPUS_BANDWIDTH_FULLBAND)); //OPUS_AUTO
    opus_multistream_encoder_ctl(_hd, OPUS_SET_VBR(!!(int)_useVbr));
    opus_multistream_encoder_ctl(_hd, OPUS_SET_BITRATE((int)_bitRate));
    opus_multistream_encoder_ctl(_hd, OPUS_SET_COMPLEXITY((int)_complexity));
    _pcmFrameSample = _fs * _frame0p1Ms / 10000;
    _pcmFrameByte = _ch * 2 * _pcmFrameSample;
    _encodedFrameByte = _bitRate * _frame0p1Ms / 80000;
    return OPUS_RET_SUCCESS;
}
OpusRet_t OpusMSEnc_c::Set(uint32_t key, void* val) noexcept {
    switch (key) {
    case Str2Key("fs"): {
        _fs = (uint32_t)val;
        break;
    ...
    case Str2Key("bitrate"): {
        ...
        if (_hd) opus_multistream_encoder_ctl(_hd, OPUS_SET_BITRATE((int)_bitRate));
        break;
    }
    case Str2Key("f0p1ms"): {
    ...
    case Str2Key("vbr"): {
        ...
        if (_hd) opus_multistream_encoder_ctl(_hd, OPUS_SET_VBR(!!(int)_useVbr));
        break;
    }
    case Str2Key("cpx"): {
        _complexity = (uint32_t)val;
        if (_hd) opus_multistream_encoder_ctl(_hd, OPUS_SET_COMPLEXITY((int)_complexity));
        break;
    }
    case Str2Key("encmode"): {
        _encMode = (int32_t)val;
        if (_hd) opus_multistream_encoder_ctl(_hd, OPUS_SET_FORCE_MODE(_encMode));
        break;
    }
    case Str2Key("outch"): {
        _ch = (uint32_t)val;
        if (_hd) opus_multistream_encoder_ctl(_hd, OPUS_SET_FORCE_CHANNELS(_ch));
        break;
    }
    case Str2Key("app"): {
        _application = (uint32_t)val;
        if (_hd) opus_multistream_encoder_ctl(_hd, OPUS_SET_APPLICATION(_application));
        break;
    }
...
OpusRet_t OpusMSEnc_c::Get(uint32_t key, void* val) noexcept {
    return OPUS_RET_SUCCESS;
}
OpusRet_t OpusMSEnc_c::Run(OpusData_c& iData, OpusData_c& oData) noexcept {
    if (_isWithHead) {
        return RrunWithHead(iData, oData);
    }
    return RunNoHead(iData, oData);
}
OpusRet_t OpusMSEnc_c::Close() noexcept {
    if (_hd) {
        opus_multistream_encoder_destroy(_hd);
    }
    return OPUS_RET_SUCCESS;
}
