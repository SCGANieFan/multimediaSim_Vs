    ...
    size_t* length_change_samples) 
{
  ...
  static const size_t k10ms = 80;  // 10 ms = 80 samples at 8 kHz sample rate.
  if (num_channels_ == 0 ||
      input_sample / num_channels_ < (2 * k10ms - 1) * fs_mult_ ||
      old_data_length >= input_sample / num_channels_ - overlap_samples_) {
    output->PushBackInterleaved(input, input_sample);
    return kError;
  }
  const bool kFastMode = false;  // Fast mode is not available for PE Expand.
  return TimeStretch::Process(input, input_sample, kFastMode, output,
...
PreemptiveExpand::ReturnCodes PreemptiveExpand::CheckCriteriaAndStretch(
  ...
  // 80 corresponds to 15 ms.
  size_t fs_mult_80 = static_cast<size_t>(fs_mult_ * 80);
  // Check for strong correlation (>0.9 in Q14) and at least 15 ms new data,
  // or passive speech.
  if (((best_correlation > kCorrelationThreshold) &&
      (old_data_length_per_channel_ <= fs_mult_80)) ||
      !active_speech) {
...
PreemptiveExpand* PreemptiveExpandFactory::Create(
  ...
  return new PreemptiveExpand(
      sample_rate_hz, num_channels, overlap_samples);
}
