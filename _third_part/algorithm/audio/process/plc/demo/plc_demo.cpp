...
#if WIN32
//#define LOG(fmt,...) 
#define LOG(fmt,...) TRACE(0, "<%s>(%s)[%d]" fmt, strrchr(__FILE__, '\\') + 1, __func__, __LINE__, ##__VA_ARGS__)
#else
#define LOG(fmt,...) TRACE(0, "<%s>(%s)[%d]" fmt, strrchr(__FILE__, '/') + 1, __func__, __LINE__, ##__VA_ARGS__)
#endif
...
#else
	ret |= plc_api_set(plc_id, "paramSet", (void*)"keyapp");//keyapp,param
	ret |= plc_api_set(plc_id, "key", (void*)(uint32_t)0);
	ret |= plc_api_set(plc_id, "app", (void*)"voice");
#endif
...
bool plc_api_demo_run(uint32_t id, uint8_t* pmc_in, int32_t* pmc_in_byte, uint8_t* pcm_out, int32_t* pcm_out_byte, uint16_t is_lost) {
	plc_api_ret_t  ret = plc_api_run(id, pmc_in, pmc_in_byte, pcm_out, pcm_out_byte, is_lost);
...
void PlcDemo0() {
	...
	GadfThreadStart("pd0", 0, plc_api_demo_thread, (void*)&params[0], 4 * 1024);

	params[1].info = gadf_get_register_info();
	params[1].isFinish = false;
	GadfThreadStart("pd1", 0, plc_api_demo_thread, (void*)&params[1], 4 * 1024);
#if WIN32
	while (1) {
		bool isFinish = true;
	...
	ASSERT(isFinish, "plc test fail");
#endif
}

EXTERNC void PlcDemo() {
	PlcDemo0();
}
