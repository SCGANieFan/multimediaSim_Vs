...
#include "music_plc_product.h"
#include "music_plc_buffer_generate.h"
#include "music_plc_audio_data.h"
#include "music_plc_math.h"
...
public:
	...
	~muter_c() {}
public:
	virtual void init(plc_memory_manger_c* mm, audio_info_c* info, muter_window_choose_e window_choose, muter_dir_choose_e dir_choose, i32 decay_samples) = 0;
	virtual void init(muter_c* muter, muter_dir_choose_e dir_choose) = 0;
	void setDir(muter_dir_choose_e dir, i16 ch) { _dir[ch] = dir; }
	void setDir(muter_dir_choose_e dir) { setDir(dir, 0); }
	void sync(muter_c *dst, i16 ch) {
	...
	void sync(muter_c *dst) {
		sync(dst, 0);
	}
	void reset(muter_dir_choose_e dir, i16 ch) {
		_dir[ch] = dir;
	...
	void reset(muter_dir_choose_e dir) {
		reset(dir, 0);
	}
	b1 is_mute_finish(i16 ch) {
		if (_dir[ch] == muter_dir_choose_e::MUTER_DIR_CHOOSE_ATTENUATION)
	...
	virtual void doMute(void* src, const i32 productSample, i16 ch) = 0;
	virtual void doMute(void* src, const i32 productSample) = 0;
public:
	u8* _mute_factor;
...
template<class T>
class muter_instance_c:public muter_c
{
public:
	muter_instance_c() {}
	~muter_instance_c() {}
public:
	virtual void init(plc_memory_manger_c* mm, audio_info_c* info, muter_window_choose_e window_choose, muter_dir_choose_e dir_choose, i32 decay_samples) override {
		_info = info;
	...
	virtual void init(muter_c* muter, muter_dir_choose_e dir_choose) override {
		_info = muter->_info;
	...
	virtual void doMute(void* src, const i32 productSample, i16 ch) override {
#if 1
#if 0
		{static int num = 0; 
	...
	virtual void doMute(void* src, const i32 productSample) override {
#if 1
