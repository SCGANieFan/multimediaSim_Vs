...
namespace webrtc {
    ...
    AudioVector::AudioVector()
        : array_((int16_t *)Alloc(kDefaultInitialSize * sizeof(int16_t))),
        first_free_ix_(0),
    ...
    AudioVector::AudioVector(size_t initial_size)
        : array_((int16_t *)Alloc(initial_size * sizeof(int16_t))),
    ...
    void AudioVector::InsertAt(
        const int16_t* insert_this,
        size_t length,
        size_t position) 
    {
        Reserve(Size() + length);
        ...
        memmove(insert_position_ptr + length, insert_position_ptr, samples_to_move * sizeof(int16_t));
        memcpy(insert_position_ptr, insert_this, length * sizeof(int16_t));
        first_free_ix_ += length;
    }

    void AudioVector::InsertZerosAt(
        size_t length,
        size_t position) 
    {
        Reserve(Size() + length);        
        ...
        size_t position) 
    {
        ...
        if (position + length > Size()) {
            ...
            first_free_ix_ += position + length;
        }
    }

    void AudioVector::CrossFade(
        const AudioVector& append_this,
        size_t fade_length) 
    {    
    ...
    void AudioVector::Reserve(size_t n) {
        if (capacity_ < n) {
            // rtc::scoped_ptr<int16_t[], rtc::SmfDeleter<int16_t[]>> temp_array(new int16_t[n]);
            rtc::scoped_ptr<int16_t[]> temp_array((int16_t *)Alloc(n * sizeof(int16_t)));
