...
namespace webrtc {
    ...
    AudioMultiVector::AudioMultiVector(size_t N, size_t initial_size) {
        ...
        for (size_t n = 0; n < N; ++n) {
            ...
            auto tmp = new AudioVector(initial_size);
            channels_[n] = tmp;
        }
        num_channels_ = N;
    }
    ...
    AudioMultiVector::~AudioMultiVector() {
        ...
        //    ++it;
        //}
        for (int i = 0; i < sizeof(channels_) / sizeof(channels_[0]); i++) {
            if (channels_[i]) {
                delete channels_[i];
                channels_[i] = 0;
            }
        ...
        size_t length)
    {
        ...
        int16_t* temp_array = (int16_t*)Alloc(length_per_channel * sizeof(int16_t));
    ...
    void AudioMultiVector::PushBackFromIndex(
        const AudioMultiVector& append_this,
        size_t index)
    {
    ...
    size_t AudioMultiVector::ReadInterleaved(
        size_t length,
    ...
    size_t AudioMultiVector::ReadInterleavedFromIndex(
        size_t start_index,
        size_t length,
        int16_t* destination) const 
    {
    ...
    size_t AudioMultiVector::ReadInterleavedFromEnd(
        size_t length,
        int16_t* destination) const 
    {
    ...
    void AudioMultiVector::OverwriteAt(
    ...
    void AudioMultiVector::CrossFade(
        const AudioMultiVector& append_this,
        size_t fade_length)
    {
        assert(num_channels_ == append_this.num_channels_);
