...
{  
  size_t fs_mult_80 = static_cast<size_t>(fs_mult_ * 80);  // Corresponds to 10 ms.
  const int16_t* signal;
  rtc::scoped_ptr<int16_t[]> signal_array;
  size_t signal_len;
  if (num_channels_ == 1) {
    signal = input;
    signal_len = in_sample;
  } else {
    ...
    signal_len = in_sample / num_channels_;
    // signal_array.reset(new int16_t[signal_len]);
    using namespace smf;
    signal_array.reset((int16_t *)Alloc(signal_len * sizeof(int16_t)));
  ...
  // |vec1| starts at 10 ms minus one pitch period.
  const int16_t* vec1 = &signal[fs_mult_80 - peak_index];
  // |vec2| start at 10 ms.
  const int16_t* vec2 = &signal[fs_mult_80];
  ...
  ReturnCodes return_value =
      CheckCriteriaAndStretch(input, in_sample, peak_index, best_correlation,
