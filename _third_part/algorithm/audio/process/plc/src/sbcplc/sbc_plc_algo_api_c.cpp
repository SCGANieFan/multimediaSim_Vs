...
plc_api_ret_t sbc_plc_algo_api_com_c::create(void** pHd, plc_api_param_t* plc_api_param, plc_base_port_c* plc_base_port) {
	...
	if (plc_api_param->frame_samples < 1
		...
		return SBC_PLC_API_RET_FAIL;
	sbc_plc_algo_api_com_c* plc_api = create_hd(plc_base_port);
	if (!plc_api) {
		return SBC_PLC_API_RET_FAIL;
	}
...
plc_api_ret_t sbc_plc_algo_api_com_c::set(void* hd, plc_api_set_e choose, void* val) {
	return SBC_PLC_API_RET_SUCCESS;
}

plc_api_ret_t sbc_plc_algo_api_com_c::get(void* hd, plc_api_get_e choose, void* val) {
	return SBC_PLC_API_RET_SUCCESS;
}

plc_api_ret_t sbc_plc_algo_api_com_c::run(void* hd, uint8_t* in, int32_t inLen, int32_t* inUsed, uint8_t* out, int32_t* outLen, uint16_t is_lost)
{
#if 0
	if (is_lost == false) {
		if (!in
	...
	if (*outLen < frame_samples * _info._bytes_per_sample)
		return SBC_PLC_API_RET_FAIL;
#endif
	sbc_plc_algo_api_com_c* plc = (sbc_plc_algo_api_com_c*)hd;
...
void plc_register_sbc_plc_int32() {
	plc_register_c::Register<music_plc_c<int16_t>>("mplc_i32");
}
void plc_register_sbc_plc_intf32() {
	plc_register_c::Register<music_plc_c<int16_t>>("mplc_if32");
}
#endif
}